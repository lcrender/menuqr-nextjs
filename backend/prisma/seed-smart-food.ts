...
import {
  SMART_FOOD_MENUS,
  SMART_FOOD_RESTAURANT,
  SmartFoodItem,
  SmartFoodMenu,
} from './data/smart-food-data';
...
function parseEmailArg(): string {
  ...
  if (!email) {
    ...
    console.error(
      '   Ejemplo: npx ts-node --project prisma/tsconfig.json prisma/seed-smart-food.ts usuario@ejemplo.com',
    );
    process.exit(1);
  }
...
async function main() {
  ...
  const restaurantConfig = SMART_FOOD_RESTAURANT;
  ...
  if (restaurant) {
    console.log(
      `ℹ️  Restaurante "${restaurantConfig.name}" ya existe (slug: ${restaurantConfig.slug}). Se reutilizará.`,
    );
  } else {
  ...
  await prisma.$executeRawUnsafe(
    `UPDATE restaurants SET template_config = $1::jsonb WHERE id = $2`,
    JSON.stringify(restaurantConfig.templateConfig),
    restaurant.id,
  );

  for (const menuConfig of SMART_FOOD_MENUS) {
    console.log('');
    console.log(`➡️  Sembrando menú: ${menuConfig.name}`);
    await seedMenu(tenantId, restaurant.id, menuConfig, iconMap);
  }

  const totalItems = SMART_FOOD_MENUS.reduce(
    (acc, m) => acc + m.sections.reduce((a, s) => a + s.items.length, 0),
    0,
  );

  console.log('');
  console.log('🎉 Seed Smart Food completado.');
  console.log(`   Restaurante: ${restaurantConfig.name} → /r/${restaurantConfig.slug}`);
  console.log(`   Menús: ${SMART_FOOD_MENUS.map((m) => m.slug).join(', ')}`);
  console.log(`   Productos totales (todas las cartas): ${totalItems}`);
  console.log(
    `   Plantilla: ${restaurantConfig.template} | Moneda: ${restaurantConfig.defaultCurrency}`,
  );
  console.log(
    '   Nota: la demo de preview no incluye fotos de producto; solo logo en /preview/logo-smart-food.png',
  );
}
