...
import { exportCoverWebpFile } from '../lib/logo-image-export';
...
export default function CoverCropModal({ show, imageSrc, onClose, onComplete }: Props) {
  ...
  const handleConfirm = async () => {
    ...
    const frameAspect =
      cs && cs.height > 0
        ? Math.abs(cs.width / cs.height - COVER_ASPECT) < 1e-4
          ? COVER_ASPECT
          : cs.width / cs.height
        : COVER_ASPECT;
    const pixelCrop =
    ...
    setError(null);
    try {
      const file = await exportCoverWebpFile(imageSrc, pixelCrop);
      onComplete(file);
      onClose();
    } catch (e: unknown) {
      ...
      <div className="modal-dialog modal-lg modal-dialog-centered">
        <div className="modal-content">
          <div className="modal-header">
            <h5 className="modal-title">Recortar portada</h5>
            <button type="button" className="btn-close" aria-label="Cerrar" onClick={handleClose} disabled={busy} />
          </div>
          <div className="modal-body">
            <p className="text-muted small mb-2">
              Encuadre para una portada en <strong>1200×800</strong> px. Se guarda en <strong>WebP</strong> con máx. <strong>600 KB</strong>.
            </p>
          ...
          <div className="modal-footer">
            ...
            <button type="button" className="btn btn-primary" onClick={handleConfirm} disabled={busy || !croppedAreaPixels}>
              {busy ? 'Procesando…' : 'Guardar portada'}
            </button>
          </div>
        </div>
      </div>
